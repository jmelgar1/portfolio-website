...
import { createScene, addCosineTreeArrangement } from '../utils/sceneBuilder';

// Helper function to build forest scene programmatically
const buildForestScene = () => {
  const sceneBuilder = createScene(
    "Forest Scene", 
    "A carefully crafted forest scene with cosine-shaped tree arrangement around valley edges"
  );

  // Add cosine-shaped tree arrangement around valley edges
  addCosineTreeArrangement(sceneBuilder, {
    valleyWidth: 50,
    centerDepth: -20,
    edgeDepth: 1.2,
    treesPerSide: 5,
    centerTrees: 3,
    thickness: 3,          // 3 layers deep for realistic forest clusters
    minTreeDistance: 3.0,  // Minimum 3 units between trees to prevent clipping
    scaleRange: [1.9, 2.7],
    layer: 'background',
    treeVariety: ['fir_007', 'fir_009', 'fir_040', 'fir_042']
  });

  // Add remaining scene elements manually (rocks, plants, flowers, grass)
  ...
  // Add all manual elements to the scene
  manualElements.forEach(element => sceneBuilder.addElement(element));

  return sceneBuilder.build();
};

export const SCENE_CONFIGS = {
  forestScene: buildForestScene(),
