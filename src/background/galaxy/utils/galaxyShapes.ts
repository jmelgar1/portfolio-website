...
export function generateSpiralGalaxy(): GalaxyPositions {
  ...
  for (let i = 0; i < NUM_STARS; i++) {
    ...
    const radius = Math.pow(Math.random(), SPIRAL.ARMS_CONSTANT) * (SPIRAL.ARM_X_MEAN / 25);
    ...
    const randomX = Math.pow(Math.random(), SPIRAL.ARMS_CONSTANT) *
      (Math.random() < 0.5 ? 1 : -1) * radius * SPIRAL.RANDOMNESS;
    const randomY = Math.pow(Math.random(), SPIRAL.ARMS_CONSTANT) *
      (Math.random() < 0.5 ? 1 : -1) * radius * SPIRAL.RANDOMNESS;
    const randomZ = Math.pow(Math.random(), SPIRAL.ARMS_CONSTANT) *
      (Math.random() < 0.5 ? 1 : -1) * radius * SPIRAL.RANDOMNESS;
...
export function generateIrregularGalaxy(): GalaxyPositions {
  ...
  for (let i = 0; i < NUM_STARS; i++) {
    ...
    if (Math.random() < 0.7) {
      ...
      positions[i3] = (Math.random() - 0.5) * range * IRREGULAR.ASYMMETRY_FACTOR;
...
export function generateGalaxyShape(type: GalaxyType): GalaxyPositions {
  ...
  console.log(`${type} sample positions:`, 
    result.positions.slice(0, 9), // First 3 particles
    'colors:', result.colors.slice(0, 9)
  );
...
export function interpolatePositions(
  ...
  for (let i = 0; i < fromPositions.length; i++) {
    result[i] = fromPositions[i] + (toPositions[i] - fromPositions[i]) * progress;
  }
