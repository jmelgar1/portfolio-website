...
describe("HolographicPanel", () => {
  ...
  test("renders without crashing", async () => {
    await expect(
      ReactThreeTestRenderer.create(<HolographicPanel {...defaultProps} />),
    ).resolves.not.toThrow();
  });

  test("renders with correct structure", async () => {
    const renderer = await ReactThreeTestRenderer.create(
      <HolographicPanel {...defaultProps} />,
    );
  ...
  test("renders with HolographicMaterial", async () => {
    // @ts-ignore
    const HolographicMaterial = (await import("./HolographicMaterial.tsx"))
      .default;
    const renderer = await ReactThreeTestRenderer.create(
      <HolographicPanel {...defaultProps} />,
    );
  ...
  test("handles click events", async () => {
    ...
    const renderer = await ReactThreeTestRenderer.create(
      <HolographicPanel {...defaultProps} onClick={mockOnClick} />,
    );
    const group = renderer.scene.children[0];
